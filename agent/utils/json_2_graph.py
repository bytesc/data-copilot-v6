...
def visualize_opensearch_result(query_json: Dict[str, Any],
    ...
    """
    Main function to visualize OpenSearch statistical analysis results.
    Smartly selects chart type based on data structure.
    """
    ...
    # Extract metadata for labels and title
    ...
    if query_type == 'stats':
        ...
        try:
            plot_stats_chart(ax, result_json, config, chart_type)
    ...
    # For distribution type - parse data first
    ...
    try:
        if strategy == 'grouped_bar':
            ...
            if strategy == 'histogram':
                plot_histogram(ax, df, primary_metric)
            elif strategy == 'bar_with_pie' or (strategy == 'single_bar' and has_percentage):
                plot_single_bar(ax, df, primary_metric, show_pie=True)
            else:
                plot_single_bar(ax, df, primary_metric, show_pie=False)
